<template>
  ...
  <div
    class="tw-upload"
    :class="{xpicture: listType==='picture', xdisabled: disabled && disabledCssHidden}"
  >
    <el-upload
      ref="upload"
      list-type="picture"
    ...
    <ul v-if="showFileList" class="tw-upload-list" :class="{xtext: listType==='text'}">
      <li ref="upload-btn-box" v-if="listType==='picture-card'" class="tw-upload-list-item"></li>
      <li
      ...
      >
        ...
        <div v-if="listType==='text'" class="tw-upload-file xtext">
          <div class="tw-upload-file-imgbox" @click="preview(file)">
            <img
              ...
              :src="/^blob:/.test(file.url) ? file.url:`${file.url}?width=24&height=24`"
            />
            <i v-else class="tw-ico" :class="['x24 xr'+file.fileType]" @click="preview(file)"></i>
            <i
              v-if="file.status!=='success'"
              class="tw-upload-percentage"
          ...
          <div v-if="!(disabled && disabledCssHidden)" class="tw-upload-file-tools">
        ...
        <!-- 卡片式 -->
        <div v-else class="tw-upload-file xcard" @click="preview(file)">
          <img
            v-if="file.fileType==='image'"
            :src="/^blob:/.test(file.url) ? file.url:`${file.url}`"
          />
          <i v-else class="tw-ico" :class="['x60 xr'+file.fileType]"></i>
          <i
            v-if="file.status!=='success'"
            class="tw-upload-percentage"
            :style="{left: file.percentage + '%'}"
          ></i>
          <a
            v-if="!(disabled && disabledCssHidden)"
            class="tw-ico xuldel1"
    ...
    <!-- <tw-previewer :visible.sync="previewerVisible"
      :images="images"
    :initIndex="previewerIndex" />-->

    <div v-if="showCover" class="tw-upload-cover" @click="showUploadingTip"></div>
  </div>
</template>
...
export default {
  ...
  computed: {
    displayFiles () {
      // return this.files.filter(file => (file.status === 'success' && !file.url.includes('blob:http:')) || (file.status !== 'success' && file.percentage > 0))
    ...
    */
    preview (file) {
      if (file.fileType === 'image') {
        ...
        this.$ui.downloadFile(file.url, file.name)
      }
    ...
    showUploadingTip () {
      this.$message.warning('文件正在上传中，请等待上传完成再进行其他操作。')
    }
  },
